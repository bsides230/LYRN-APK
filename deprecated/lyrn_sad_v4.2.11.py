...
class LyrnAIInterface(ctk.CTkToplevel):
    ...
    def _ensure_active_jobs_index(self):
        ...
        active_jobs_index_path = Path(SCRIPT_DIR) / "build_prompt" / "active_jobs" / "_index.json"
        active_jobs_dir = active_jobs_index_path.parent
        active_jobs_dir.mkdir(parents=True, exist_ok=True)
        if not active_jobs_index_path.exists():
            with open(active_jobs_index_path, 'w', encoding='utf-8') as f:
                json.dump([], f)
    ...
    def _load_active_jobs(self) -> List[str]:
        ...
        if not active_jobs_index_path.exists():
            return []
        try:
            with open(active_jobs_index_path, 'r', encoding='utf-8') as f:
                paths = json.load(f)
            job_names = []
    ...
    def update_active_job_files(self, active_job_names: List[str]):
        ...
        # Remove inactive
        for item in active_jobs_dir.iterdir():
        ...
        for job_name in active_job_names:
            if not self.automation_controller: continue

            job_definition = self.automation_controller.job_definitions.get(job_name)
            if not job_definition: continue

            instructions = job_definition.get("instructions", "")
            if not instructions: continue

            job_dir = active_jobs_dir / job_name
            job_dir.mkdir(exist_ok=True)

            try:
                with open(job_dir / "instructions.txt", 'w', encoding='utf-8') as f:
