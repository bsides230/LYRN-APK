"""
LYRN-AI Interface v6.2 - Advanced GUI with Full Feature Set
...
class TabbedSettingsDialog(ctk.CTkToplevel):
    ...
    def create_theme_builder_tab(self):
        """Create the theme builder tab UI."""
        try:
            font = ctk.CTkFont(family="Consolas", size=12)
            title_font = ctk.CTkFont(family="Consolas", size=14, weight="bold")
        except:
        ...
        # Frame for the builder
        builder_frame = ctk.CTkFrame(self.tab_theme_builder)
        builder_frame.pack(expand=True, fill="both", padx=20, pady=10)

        # Theme Name
        ctk.CTkLabel(builder_frame, text="Theme Name:", font=font).pack(anchor="w", padx=10, pady=(10, 0))
        self.theme_name_entry = ctk.CTkEntry(builder_frame, font=font)
        ...
        color_labels = {
            ...
            "textbox_fg": "Textbox Text",
            "label_text": "Label Text",
            "system_text": "System Text Color",
            "user_text": "User Text Color",
            "assistant_text": "Assistant Text Color",
            "thinking_text": "Thinking Text Color",
            "display_text_color": "Display Text Color"
        }

        for key, label in color_labels.items():
            frame = ctk.CTkFrame(builder_frame)
            frame.pack(fill="x", padx=10, pady=5)
            ctk.CTkLabel(frame, text=label, font=font).pack(side="left", padx=10)
            entry = ctk.CTkEntry(frame, font=font)
            entry.pack(side="right", fill="x", expand=True, padx=10)
            self.color_entries[key] = entry

        # Preview area
        self.preview_frame = ctk.CTkFrame(builder_frame, height=150)
        self.preview_frame.pack(fill="x", padx=10, pady=10)

        self.preview_label = ctk.CTkLabel(self.preview_frame, text="Theme Preview", font=title_font)
        self.preview_label.pack(pady=5)

        self.preview_button = ctk.CTkButton(self.preview_frame, text="Button")
        self.preview_button.pack(pady=5)

        self.preview_textbox = ctk.CTkTextbox(self.preview_frame, height=50)
        self.preview_textbox.insert("0.0", "This is a textbox.")
        self.preview_textbox.pack(pady=5, padx=10, fill="x")

        self.glass_effect_var = ctk.BooleanVar()
        ctk.CTkCheckBox(builder_frame, text="Enable Glass Effect", font=font, variable=self.glass_effect_var).pack(anchor="w", padx=10, pady=10)

        # Buttons
        button_frame = ctk.CTkFrame(builder_frame)
    ...
    def preview_theme(self):
        """Updates the preview area with the current colors."""
        colors = {key: entry.get() for key, entry in self.color_entries.items()}

        self.preview_frame.configure(fg_color=colors.get("frame_bg", "#FFFFFF"))
        self.preview_label.configure(text_color=colors.get("label_text", "#000000"))
        self.preview_button.configure(fg_color=colors.get("primary", "#0000FF"))
        self.preview_textbox.configure(fg_color=colors.get("textbox_bg", "#FFFFFF"),
                                       text_color=colors.get("textbox_fg", "#000000"))
...
class LyrnAIInterface(ctk.CTk):
    ...
    def setup_window(self):
        ...
        self.title("LYRN-AI Interface v6.2")
    ...
    def create_left_sidebar(self):
        ...
        self.load_model_button = ctk.CTkButton(system_frame, text="📦 Load Model",
        ...
        self.offload_model_button = ctk.CTkButton(system_frame, text="📤 Offload Model",
        ...
        self.settings_button = ctk.CTkButton(system_frame, text="⚙ Settings",
        ...
        self.show_llm_log_button = ctk.CTkButton(system_frame, text="📋 Show LLM Log",
        ...
        self.clear_chat_button = ctk.CTkButton(system_frame, text="📁 Clear Chat",
...
def main():
    """Main entry point for LYRN-AI v6.2"""
    log_queue = queue.Queue()
    redirector = ConsoleRedirector(log_queue)
    redirector.start()

    try:
        print("Starting LYRN-AI Interface v6.2...")
