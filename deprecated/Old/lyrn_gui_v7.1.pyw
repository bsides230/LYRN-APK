...
class LyrnAIInterface(ctk.CTkToplevel):
    ...
    def setup_model(self):
        """Initialize LLM model as a separate process."""
        if not self.settings_manager.settings:
        ...
        try:
            ...
            command = [
                ...
                "--n_ctx", str(active.get("n_ctx", 8192)),
                "--n_threads", str(active.get("n_threads", 8)),
                "--n_gpu_layers", str(active.get("n_gpu_layers", 0)),
            ]

            print(f"Starting model process with command: {' '.join(command)}")
            ...
            time.sleep(4) # Give it a moment to load/fail
            if self.model_process.poll() is not None:
                stdout, stderr = self.model_process.communicate()
                raise RuntimeError(f"Model loader process failed to start.\nStderr: {stderr}\nStdout: {stdout}")

            print("Model loader process started successfully.")
            self.stream_queue.put(('status_update', 'Model Loaded', LYRN_SUCCESS))
            self.set_model_status("Ready")

        except Exception as e:
            print(f"Error setting up model process: {e}")
            self.model_process = None
            self.stream_queue.put(('status_update', 'Model Load Failed', LYRN_ERROR))
...
"""
    ...
    def offload_model(self):
        ...
        try:
            ...
            print(f"Could not terminate model process gracefully: {e}")
            if self.model_process:
                try:
                    self.model_process.kill()
                    print("Model process killed.")
                except Exception as kill_e:
                    print(f"Failed to kill model process: {kill_e}")
        finally:
            self.model_process = None
    ...
    def send_message(self):
        ...
        if not self.model_process or self.model_process.poll() is not None:
            self.update_status("Model not loaded or process not running.", LYRN_ERROR)
            self.set_model_status("Model Error")
            return
