...
class SystemResourceMonitor:
    ...
    def get_stats(self) -> Dict[str, any]:
        ...
        # Get HWiNFO data
        try:
            hwinfo_data = hwinfo_monitor.get_hwinfo_data()
            stats["hwinfo"] = hwinfo_data
            if hwinfo_data is None:
                # This is a common case, so we don't spam the log.
                # A message will be shown in the GUI.
                pass
        except Exception as e:
            print(f"ERROR: An unexpected error occurred while getting HWiNFO data: {e}")
            stats["hwinfo"] = None
...
class TabbedSettingsDialog(ctk.CTkToplevel):
    ...
    def create_widgets(self):
        ...
        self.tab_theme_builder = self.tabview.add(self.language_manager.get("tab_theme_builder"))
    ...
    def create_ui_settings_tab(self):
        ...
        # --- Window Style ---
        window_style_frame = ctk.CTkFrame(self.tab_ui_settings)
        window_style_frame.pack(fill="x", padx=20, pady=10)
        ctk.CTkLabel(window_style_frame, text="Window Style", font=title_font).pack(pady=10, anchor="w", padx=10)

        self.windows_style_controls_var = ctk.BooleanVar(value=self.settings_manager.ui_settings.get("windows_style_controls", False))
        ctk.CTkSwitch(window_style_frame, text="Windows-style window controls (requires restart)", variable=self.windows_style_controls_var, font=font).pack(anchor="w", padx=10, pady=10)
        ...
        edit_language_button = ctk.CTkButton(lang_select_frame, text="Edit Translations", font=font, command=self.open_language_editor)
        edit_language_button.pack(side="left", padx=10)
        Tooltip(edit_language_button, "Open an editor for the selected language file.")
    ...
    def create_theme_builder_tab(self):
        ...
        for key, label in color_labels.items():
            ...
            frame.pack(fill="x", padx=10, pady=2, ipady=2)
            ctk.CTkLabel(frame, text=label, font=font, width=120, anchor="w").pack(side="left", padx=5)
            entry = ctk.CTkEntry(frame, font=font)
            entry.pack(side="left", fill="x", expand=True, padx=5)

            color_button = ctk.CTkButton(frame, text="...", width=25, command=lambda e=entry: self.choose_color(e))
            color_button.pack(side="left", padx=(0, 5))

            entry.bind("<KeyRelease>", lambda event: self.preview_theme(), add="+")
    ...
    def choose_color(self, entry_widget):
        """Opens a color chooser dialog and updates the given entry widget."""
        color_code = colorchooser.askcolor(title="Choose color")
        if color_code and color_code[1]:  # color_code is a tuple ((r,g,b), '#rrggbb')
            entry_widget.delete(0, "end")
            entry_widget.insert(0, color_code[1])
            self.preview_theme() # Update the preview after choosing a color
    ...
    def apply_preview_theme(self):
        ...
        # 1. Collect the colors from the entry widgets
        preview_colors = {}
        for key, entry in self.color_entries.items():
            color_val = entry.get()
            if color_val: # Only include non-empty values
                preview_colors[key] = color_val
    ...
    def preview_theme(self):
        ...
        colors = {key: entry.get() for key, entry in self.color_entries.items() if entry.get()}
    ...
    def load_selected_theme(self, theme_name: str):
        ...
        for key, entry in self.color_entries.items():
            entry.delete(0, "end")
            entry.insert(0, theme_colors.get(key, ""))
    ...
    def save_theme(self):
        ...
        for key, entry in self.color_entries.items():
            theme_data["colors"][key] = entry.get()
    ...
    def apply_theme(self):
        ...
        self.preview_theme() # Refresh preview
    ...
    def save_all_settings(self):
        ...
        try:
            ...
            self.settings_manager.ui_settings["windows_style_controls"] = self.windows_style_controls_var.get()
    ...
    def open_language_editor(self):
        """Opens the language editor popup."""
        selected_language = self.language_var.get()
        if not hasattr(self, 'language_editor_popup') or not self.language_editor_popup.winfo_exists():
            self.language_editor_popup = LanguageEditorPopup(self, self.language_manager, selected_language, self.theme_manager)
            self.language_editor_popup.focus()
        else:
            self.language_editor_popup.lift()
            self.language_editor_popup.focus()

class LanguageEditorPopup(ctk.CTkToplevel):
    """A popup window for editing language files."""
    def __init__(self, parent, language_manager, language_code, theme_manager):
        super().__init__(parent)
        self.language_manager = language_manager
        self.language_code = language_code
        self.language_file_path = os.path.join(self.language_manager.language_dir, f"{self.language_code}.json")

        self.title(f"Language Editor - {language_code}")
        self.geometry("800x600")
    ...
    def create_widgets(self):
        """Create the widgets for the editor."""
        scroll_frame = ctk.CTkScrollableFrame(self, label_text="Translations")
        scroll_frame.pack(expand=True, fill="both", padx=10, pady=10)

        save_button = ctk.CTkButton(self, text="Save Changes", command=self.save_language_file)
        save_button.pack(pady=10)

        self.scroll_frame = scroll_frame

    def load_language_file(self):
        """Load the language file and create entry widgets."""
        ...
        for key, value in sorted(strings.items()):
            ...
            label = ctk.CTkLabel(frame, text=key, width=200, anchor="w")
    ...
    def save_language_file(self):
        ...
        for key, entry in self.entries.items():
            new_strings[key] = entry.get()

        try:
            with open(self.language_file_path, 'w', encoding='utf-8') as f:
                json.dump(new_strings, f, indent=2, ensure_ascii=False)

            # Reload the language in the main app to reflect changes if it's the current language
            if self.language_code == self.language_manager.language:
                self.language_manager.load_language()

            print(f"Language file '{self.language_code}.json' saved successfully.")
            self.destroy()
        except Exception as e:
...
class LyrnAIInterface(ctk.CTkToplevel):
    ...
    def setup_window(self):
        ...
        self.title("LYRN-AI Interface v6.6")
        ...
        try:
            if Image and ImageTk:
                logo_path = os.path.join(SCRIPT_DIR, "images/lyrn_logo.jpg")
                if os.path.exists(logo_path):
                    logo_image = Image.open(logo_path)
                    # For taskbar icons, it's better to use a PhotoImage object
    ...
    def create_left_sidebar(self):
        ...
        self.job_frame = ctk.CTkFrame(self.left_sidebar, border_width=1, border_color=self.theme_manager.get_color("border_color"))
        self.job_frame.pack(fill="x", padx=10, pady=10)

        ctk.CTkLabel(self.job_frame, text="Job Automation", font=section_font).pack(pady=10)

        # Start/Stop buttons
        button_frame = ctk.CTkFrame(self.job_frame)
        ...
        Tooltip(self.start_btn, self.tooltips.get("start_jobs_button", ""))
        ...
        Tooltip(self.stop_btn, self.tooltips.get("stop_jobs_button", ""))
        ...
        self.job_dropdown = ctk.CTkComboBox(
            self.job_frame, values=self.job_processor.job_names if hasattr(self, 'job_processor') else [],
        ...
        self.system_frame = ctk.CTkFrame(self.left_sidebar, border_width=1, border_color=self.theme_manager.get_color("border_color"))
        self.system_frame.pack(fill="x", padx=10, pady=10)

        ctk.CTkLabel(self.system_frame, text="System Controls", font=section_font).pack(pady=10)

        self.load_model_button = ctk.CTkButton(self.system_frame, text="📦 Load Model",
                     font=normal_font, command=self.reload_model_full)
        self.load_model_button.pack(padx=10, pady=3, fill="x")
        Tooltip(self.load_model_button, self.tooltips.get("load_model_button", ""))

        self.offload_model_button = ctk.CTkButton(self.system_frame, text="📤 Offload Model",
                     font=normal_font, command=self.offload_model)
        self.offload_model_button.pack(padx=10, pady=3, fill="x")
        Tooltip(self.offload_model_button, self.tooltips.get("offload_model_button", ""))

        self.show_llm_log_button = ctk.CTkButton(self.system_frame, text="📋 View Logs",
        ...
        self.clear_chat_folder_button = ctk.CTkButton(self.system_frame, text="📁 Clear Chat Folder",
        ...
        self.quick_frame = ctk.CTkFrame(self.left_sidebar, border_width=1, border_color=self.theme_manager.get_color("border_color"))
    ...
    def create_right_sidebar(self):
        ...
        settings_frame = ctk.CTkFrame(self.right_sidebar)
        settings_frame.pack(side="top", fill="x", padx=10, pady=10)

        self.settings_button = ctk.CTkButton(settings_frame, text="Settings", command=self.open_settings)
    ...
    def create_hwinfo_section(self):
        ...
        self.hwinfo_frame = ctk.CTkFrame(self.right_sidebar, border_width=1, border_color=self.theme_manager.get_color("border_color"))
        self.hwinfo_frame.pack(fill="both", expand=True, padx=10, pady=10)

        ctk.CTkLabel(self.hwinfo_frame, text="HWiNFO Sensor Data", font=section_font).pack(pady=10)

        self.hwinfo_scroll_frame = ctk.CTkScrollableFrame(self.hwinfo_frame, label_text="Live Sensors")
    ...
    def create_enhanced_metrics(self):
        ...
        self.metrics_frame = ctk.CTkFrame(self.right_sidebar, border_width=1, border_color=self.theme_manager.get_color("border_color"))
    ...
    def update_system_gauges(self, stats: Dict[str, any]):
        ...
        # HWiNFO Data
        hwinfo_data = stats.get("hwinfo")
        if hwinfo_data == "ACCESS_DENIED":
            if not hasattr(self, 'hwinfo_no_data_label') or self.hwinfo_no_data_label.cget("text").startswith("HWiNFO data not found"):
                for widget in self.hwinfo_scroll_frame.winfo_children():
                    widget.destroy()
                self.hwinfo_no_data_label = ctk.CTkLabel(self.hwinfo_scroll_frame,
                                                         text="HWiNFO Access Denied.\n\nPlease try running LYRN-AI as an\nAdministrator to view sensor data.",
                                                         justify="left", anchor="w")
                self.hwinfo_no_data_label.pack(fill="x", padx=10, pady=10)
        elif hwinfo_data:
            # Data is present. Remove the 'no data' message if it exists.
            if hasattr(self, 'hwinfo_no_data_label'):
                self.hwinfo_no_data_label.destroy()
                delattr(self, 'hwinfo_no_data_label')

            # Initialize the list of labels if it doesn't exist
            if not hasattr(self, 'hwinfo_sensor_labels'):
                # First time displaying data, clear the initial "waiting" message
                for widget in self.hwinfo_scroll_frame.winfo_children():
                    widget.destroy()
                self.hwinfo_sensor_labels = []

            # Filter for interesting sensors to display
            interesting_sensors = {
                "CPU": ["CPU (Tctl/Tdie)", "CPU Fan"],
                "GPU": ["GPU Temperature", "GPU Fan"]
            }
            readings_to_display = []
            for reading in hwinfo_data:
                for sensor_type, labels in interesting_sensors.items():
                    if reading["sensor_name"] in labels:
                        readings_to_display.append(f"{reading['sensor_name']} - {reading['label']}: {reading['value']:.1f} {reading['unit']}")

            # Update the labels, creating or destroying them as needed
            for i, text in enumerate(readings_to_display):
                if i < len(self.hwinfo_sensor_labels):
                    self.hwinfo_sensor_labels[i].configure(text=text)
                else:
                    label = ctk.CTkLabel(self.hwinfo_scroll_frame, text=text, anchor="w")
                    label.pack(fill="x", padx=5, pady=2)
                    self.hwinfo_sensor_labels.append(label)

            # Remove any surplus labels if the number of readings decreased
            while len(self.hwinfo_sensor_labels) > len(readings_to_display):
                label_to_remove = self.hwinfo_sensor_labels.pop()
                label_to_remove.destroy()
        else:
            # No data. Clear any existing sensor labels and show the 'no data' message.
            if hasattr(self, 'hwinfo_sensor_labels'):
                for label in self.hwinfo_sensor_labels:
                    label.destroy()
                delattr(self, 'hwinfo_sensor_labels')

            if not hasattr(self, 'hwinfo_no_data_label'):
                # Clear initial waiting message
                for widget in self.hwinfo_scroll_frame.winfo_children():
                    widget.destroy()
                # Create the message
                self.hwinfo_no_data_label = ctk.CTkLabel(self.hwinfo_scroll_frame,
                                                         text="HWiNFO data not found.\n\nEnsure HWiNFO is running and\n'Shared Memory Support' is enabled\nin HWiNFO's main settings.",
                                                         justify="left", anchor="w")
                self.hwinfo_no_data_label.pack(fill="x", padx=10, pady=10)

    def create_enhanced_status(self):
        """Create enhanced status display with better organization."""
        self.status_frame = ctk.CTkFrame(self.left_sidebar, border_width=1, border_color=self.theme_manager.get_color("border_color"))
        self.status_frame.pack(fill="x", padx=10, pady=(0, 10))

        try:
            section_font = ctk.CTkFont(family="Consolas", size=14, weight="bold")
            status_font = ctk.CTkFont(family="Consolas", size=12, weight="bold")
            info_font = ctk.CTkFont(family="Consolas", size=10)
        except:
            section_font = ("Consolas", 14, "bold")
        ...
        # Main Status (Ready, Loading, Error)
        status_container = ctk.CTkFrame(self.status_frame, fg_color="transparent")
        status_container.pack(fill="x", padx=10, pady=5)
        self.status_indicator = ctk.CTkLabel(status_container, text="●", font=("Arial", 20), text_color=LYRN_SUCCESS)
        self.status_indicator.pack(side="left", padx=(0, 10))
        self.status_label = ctk.CTkLabel(status_container, text="LYRN-AI Ready", font=status_font, text_color=LYRN_SUCCESS)
        ...
        # Model Info Section
        model_info_frame = ctk.CTkFrame(self.status_frame, fg_color="transparent")
        model_info_frame.pack(fill="x", padx=10, pady=5, anchor="w")
        ctk.CTkLabel(model_info_frame, text="Model:", font=info_font, width=50, anchor="w").pack(side="left")
        self.model_status_label = ctk.CTkLabel(model_info_frame, text="Not Loaded", font=info_font, anchor="w")
        self.model_status_label.pack(side="left")

        # UI Info Section
        ui_info_frame = ctk.CTkFrame(self.status_frame, fg_color="transparent")
        ui_info_frame.pack(fill="x", padx=10, pady=(0,10), anchor="w")
        ctk.CTkLabel(ui_info_frame, text="Theme:", font=info_font, width=50, anchor="w").pack(side="left")
        self.theme_status_label = ctk.CTkLabel(ui_info_frame, text="Unknown", font=info_font, anchor="w")
    ...
    def create_chat_area(self):
        ...
        self.chat_display = ctk.CTkTextbox(chat_frame, font=chat_font, wrap="word", text_color=self.theme_manager.get_color("display_text_color"))
    ...
    def apply_color_theme(self):
        ...
        try:
            ...
            for widget_type, config in [
                (ctk.CTkButton, {"fg_color": primary_color}),
                (ctk.CTkComboBox, {"button_color": primary_color, "button_hover_color": accent_color}),
                (ctk.CTkFrame, {"fg_color": frame_bg}),
                (ctk.CTkLabel, {"text_color": label_text}),
                (ctk.CTkTextbox, {"fg_color": textbox_bg, "text_color": textbox_fg}),
                (ctk.CTkScrollableFrame, {"fg_color": frame_bg, "label_fg_color": primary_color})
            ]:
                for widget in self.find_widgets_recursively(self, widget_type):
                    widget.configure(**config)
            ...
            if hasattr(self, 'status_frame'):
                self.status_frame.configure(fg_color=tm.get_color("primary", "#880ED4"))
    ...
    def offload_model(self):
        ...
        def offload_thread():
            try:
                ...
                print("Model offloaded successfully.")
            except Exception as e:
                self.stream_queue.put(('status_update', f'Offload failed: {e}', LYRN_ERROR))
                print(f"Error during model offload: {e}")

        threading.Thread(target=offload_thread, daemon=True).start()
    ...
    def update_status(self, message: str, color: str = None):
        ...
        try:
            ...
            if self.llm and self.settings_manager.settings:
                ...
                self.model_status_label.configure(text=f"Loaded: {model_name}")
            else:
                self.model_status_label.configure(text="Not Loaded")

            if hasattr(self, 'theme_status_label'):
                self.theme_status_label.configure(text=self.theme_manager.get_current_theme_name())
...
def main():
    """Main entry point for LYRN-AI v6.6"""
    log_queue = queue.Queue()
    redirector = ConsoleRedirector(log_queue)
    redirector.start()

    try:
        print("Starting LYRN-AI Interface v6.6...")
