...
class JobWatcherPopup(ctk.CTkToplevel):
    ...
    def __init__(self, parent, job_watcher_manager: JobWatcherManager, theme_manager: ThemeManager, language_manager: LanguageManager):
        ...
        self.title("Job Automation Watcher")
        self.geometry("900x700")
        self.transient(parent)
        self.grab_set()
    ...
    def delete_selected_job(self):
        ...
        dialog = ctk.CTkInputDialog(text=f"Type DELETE to confirm deleting job '{self.selected_job_name}':", title="Confirm Deletion")
        confirmation = dialog.get_input()

        if confirmation == "DELETE":
            self.job_watcher_manager.delete_job(self.selected_job_name)
            self.parent_app.update_status(f"Job '{self.selected_job_name}' deleted.", LYRN_SUCCESS)
            self.refresh_job_list()
        else:
...
class LyrnAIInterface(ctk.CTkToplevel):
    ...
    def create_right_sidebar(self):
        ...
        self.job_watcher_button = ctk.CTkButton(self.job_frame, text="Job Automation Watcher", command=self.open_job_watcher_popup)
