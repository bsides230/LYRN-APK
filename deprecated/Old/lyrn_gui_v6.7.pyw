...
class LyrnAIInterface(ctk.CTkToplevel):
    ...
    def __init__(self, master, log_queue: queue.Queue):
        ...
        self.status_blinking = False
        self.status_indicator_on = True

        # Handle window closing
        self.protocol("WM_DELETE_WINDOW", self.on_closing)
        self.bind("<Control-Shift-P>", self.open_command_palette)

        self.start_blinking() # Start blinking on init

    def start_blinking(self):
        if not self.llm:
            self.status_blinking = True
            self.toggle_status_blink()

    def stop_blinking(self):
        self.status_blinking = False
        # Ensure the indicator is left in a visible state
        self.status_indicator.configure(text_color=LYRN_SUCCESS) # Default to green
        self.status_indicator_on = True

    def toggle_status_blink(self):
        if not self.status_blinking:
            return # Stop the loop

        if self.status_indicator_on:
            self.status_indicator.configure(text_color=self.cget("fg_color")) # "off" state
        else:
            self.status_indicator.configure(text_color=LYRN_ERROR) # "on" state (red)

        self.status_indicator_on = not self.status_indicator_on
        self.after(500, self.toggle_status_blink)
    ...
    def setup_model(self):
        ...
        self.stop_blinking()
        self.status_indicator.configure(text_color=LYRN_WARNING) # Yellow for loading
        ...
        try:
            ...
            self.status_indicator.configure(text_color=LYRN_SUCCESS) # Green for loaded/idle
    ...
    def create_enhanced_status(self):
        ...
        status_container.pack(fill="x", expand=True, padx=10, pady=5)

        self.status_indicator = ctk.CTkLabel(status_container, text="●", font=("Arial", 20), text_color=LYRN_ERROR)
        self.status_indicator.pack(side="left", padx=(0, 10))

        self.status_textbox = ctk.CTkTextbox(status_container, height=100, wrap="word", font=status_font)
        self.status_textbox.pack(fill="x", expand=True)
        self.status_textbox.insert("end", "Model not loaded. Please load a model to begin.")
        self.status_textbox.configure(state="disabled", text_color=LYRN_ERROR)
    ...
    def create_chat_area(self):
        ...
        self.chat_display = ctk.CTkTextbox(chat_frame, font=chat_font, wrap="word", border_width=1, text_color=self.theme_manager.get_color("display_text_color"))
        ...
        self.input_box = ctk.CTkTextbox(input_frame, height=100, font=chat_font, border_width=1)
...
"""
    ...
    def send_message(self):
        ...
        self.status_indicator.configure(text_color=LYRN_INFO) # Blue for generating
            ...
            while True:
                try:
                    ...
                    if message[0] == 'token':
                        ...
                        self.status_indicator.configure(text_color=LYRN_SUCCESS) # Green for idle
