...
class SystemResourceMonitor:
    ...
    def get_stats(self) -> Dict[str, any]:
        ...
        try:
            ...
            # Don't print this every time, it can be spammy
            # print(f"Could not get HWiNFO data: {e}")
            stats["hwinfo"] = None
...
class LogViewerPopup(ctk.CTkToplevel):
    ...
    def __init__(self, parent, log_queue: queue.Queue, settings_manager: SettingsManager):
...
class TabbedSettingsDialog(ctk.CTkToplevel):
    ...
    def preview_theme(self):
        ...
        colors = {key: entry.get() for key, entry in self.color_entries.items()}
    ...
    def delete_selected_theme(self):
        ...
        self.preview_widgets["button"].configure(fg_color=primary, text_color=textbox_bg)
    ...
    def apply_theme(self):
        ...
        # Theme the tabview
        self.tabview.configure(segmented_button_selected_color=primary_color,
        ...
        # Find all checkboxes and apply the primary color
        all_checkboxes = self.find_widgets_recursively(self, ctk.CTkCheckBox)
        for chk in all_checkboxes:
            chk.configure(fg_color=primary_color)
...
class LyrnAIInterface(ctk.CTk):
    ...
    def _toggle_maximize(self):
        if self._maximized:
            self.geometry(self._geom_before_maximize)
        else:
            self._geom_before_maximize = self.geometry()
    ...
    def open_command_palette(self, event=None):
        ...
        if hasattr(self, 'cmd_palette') and self.cmd_palette.winfo_exists():
            ...
            self.cmd_palette = CommandPalette(self, commands=commands)
            self.cmd_palette.focus()
    ...
    def setup_window(self):
        ...
        self.title("LYRN-AI Interface v6.5")
    ...
    def create_top_bar(self):
        """Creates the new top bar for window controls and dragging."""
        top_bar_color = self.theme_manager.get_color("frame_bg")
        top_bar = ctk.CTkFrame(self, height=40, corner_radius=0, fg_color=top_bar_color)
        ...
        left_controls = ctk.CTkFrame(top_bar, fg_color="transparent")
        left_controls.pack(side="left", padx=10, pady=5)

        button_font = ctk.CTkFont(family="Consolas", size=16, weight="bold")
        close_button = ctk.CTkButton(left_controls, text="✕", command=self.on_closing, width=28, height=28, corner_radius=4)
        close_button.pack(side="left", padx=4)
        minimize_button = ctk.CTkButton(left_controls, text="—", command=self.minimize_window, width=28, height=28, corner_radius=4)
        minimize_button.pack(side="left", padx=4)
        maximize_button = ctk.CTkButton(left_controls, text="□", command=self._toggle_maximize, width=28, height=28, corner_radius=4)
        maximize_button.pack(side="left", padx=4)
        ...
        self.settings_button = ctk.CTkButton(right_controls, text="⚙", command=self.open_settings, width=30)
        self.settings_button.pack()
        Tooltip(self.settings_button, self.tooltips.get("settings_button", ""))

        # Bind dragging to the container frames, NOT the buttons themselves
        for widget in [left_controls, right_controls]:
    ...
    def create_left_sidebar(self):
        ...
        try:
            if Image and ImageTk:
                ...
                if os.path.exists(logo_path):
                    ...
                    logo_photo = ImageTk.PhotoImage(logo_image.resize((24, 24)))
        ...
        self.clear_chat_button = ctk.CTkButton(self.system_frame, text="🗑️ Clear Display Text",
                     font=normal_font, command=self.clear_chat)
        self.clear_chat_button.pack(padx=10, pady=3, fill="x")
    ...
    def create_right_sidebar(self):
        ...
        # self.right_sidebar.grid(row=1, column=2, sticky="nsew", padx=(5, 10), pady=(0, 10))
    ...
    def create_hwinfo_section(self):
        ...
        print("DEBUG: create_right_sidebar END")
        return self.right_sidebar
    ...
    def create_chat_area(self):
        ...
        hint_label.grid(row=0, column=0, sticky="ne", padx=5, pady=2)
        ...
        chat_frame.bind("<ButtonPress-1>", self._start_move)
        chat_frame.bind("<B1-Motion>", self._do_move)
        input_frame.bind("<ButtonPress-1>", self._start_move)
        input_frame.bind("<B1-Motion>", self._do_move)
...
"""
    ...
    def toggle_log_viewer(self):
        ...
        if not hasattr(self, 'log_viewer_popup') or not self.log_viewer_popup.winfo_exists():
            self.log_viewer_popup = LogViewerPopup(self, self.log_queue, self.settings_manager)
...
def main():
    """Main entry point for LYRN-AI v6.5"""
    log_queue = queue.Queue()
    # redirector = ConsoleRedirector(log_queue)
    # redirector.start()

    try:
        print("Starting LYRN-AI Interface v6.5...")
