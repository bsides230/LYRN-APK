...
class LyrnAIInterface(ctk.CTkToplevel):
    ...
    def start_application_logic(self):
        ...
        self.start_pulsing() # Start pulsing on init
    ...
    def stop_pulsing(self):
        ...
        try:
            if hasattr(self, 'model_status_progress_bar'):
                self.model_status_progress_bar.configure(progress_color=LYRN_SUCCESS)
        except Exception as e:
            print(f"Error stopping pulse: {e}")
    ...
    def pulse_progress_bar(self):
        ...
        try:
            pulse_color_1 = LYRN_ERROR
            pulse_color_2 = LYRN_WARNING

            target_color = pulse_color_2 if self.pulse_state else pulse_color_1
        ...
        self.after(800, self.pulse_progress_bar)
    ...
    def create_left_sidebar(self):
        ...
        datetime_frame.grid(row=0, column=2, sticky="e", padx=10)
        self.time_label = ctk.CTkLabel(datetime_frame, text="", font=datetime_font, text_color="white")
        self.time_label.pack()
        self.date_label = ctk.CTkLabel(datetime_frame, text="", font=datetime_font, text_color="white")
    ...
    def create_enhanced_metrics(self):
        ...
        self.metrics_frame = ctk.CTkFrame(self.right_sidebar, fg_color="transparent", border_width=0)
...
"""
    ...
    def generate_response(self, user_text: str):
        ...
        try:
            ...
            # TODO: Load deltas and construct full context
            messages = [
    ...
    def _run_heartbeat_cycle(self, user_input: str, assistant_output: str):
        ...
        """
        self.stream_queue.put(('status_update', 'Running Heartbeat...', LYRN_INFO))
        try:
            if not self.llm:
                print("Heartbeat skipped: LLM not available.")
                return

            # 1. Construct the heartbeat prompt
            heartbeat_prompt = get_heartbeat_job_prompt(user_input, assistant_output)

            # 2. Call the LLM (non-streaming)
            response = self.llm.create_chat_completion(
            ...
            # 3. Save the raw output to a file for the watcher
            output_dir = Path(SCRIPT_DIR) / "automation" / "heartbeat_outputs"
            output_dir.mkdir(parents=True, exist_ok=True)

            timestamp = datetime.utcnow().strftime("%Y%m%d_%H%M%S_%f")
            output_filepath = output_dir / f"hb_{timestamp}.txt"
            ...
            self.stream_queue.put(('status_update', 'Heartbeat complete', LYRN_SUCCESS))
