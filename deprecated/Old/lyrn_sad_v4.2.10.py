...
class LyrnAIInterface(ctk.CTkToplevel):
    ...
    def __init__(self, master, log_queue: queue.Queue):
        ...
        # Start RWI Builder Server
        try:
            from modules.rwi_builder.server import start_server
            ...
            def build_wrapper():
                if self.snapshot_loader:
                    ...
                    raise Exception("System loading, please wait...")

            start_server(port=8000, build_callback=build_wrapper)
            print("RWI Builder Server started on port 8000")
        except Exception as e:
            print(f"Failed to start RWI server: {e}")

        # Start Job Manager Server
        try:
            from modules.job_manager.server import start_server as start_job_manager_server

            def run_job_callback(job_name):
                 self.after(0, lambda: self.run_selected_job_from_web(job_name))

            def run_reflection_callback(data):
                 self.after(0, lambda: self.queue_reflection_job_from_web(data))

            start_job_manager_server(
                port=8001,
                automation_controller=self.automation_controller,
                scheduler_manager=self.scheduler_manager,
                cycle_manager=self.cycle_manager,
                run_job_callback=run_job_callback,
                run_reflection_callback=run_reflection_callback
            )
            print("Job Manager Server started on port 8001")
        except Exception as e:
            print(f"Failed to start Job Manager server: {e}")
    ...
    def run_selected_job_from_web(self, job_name):
    ...
    def queue_reflection_job_from_web(self, data):
        # Injects the last assistant response as 'job_output' if not provided
        # or if we assume the workflow is: Run Job -> View Output -> Reflect.
        # But data might not have job_output if sent from web manually.
        # We'll use self.last_assistant_response

        job_output = getattr(self, 'last_assistant_response', '')
        if not job_output:
            self.update_status("No recent job output for reflection.", LYRN_WARNING)
            return
        ...
        job_args = {
            ...
            "original_job_name": data.get("original_job_name", "manual")
        }

        # Batch size is handled by client or separate logic, but reflection job itself receives iterations

        self.automation_controller.add_job(
            name="reflection_cycle_job",
            args=job_args
        )
        self.update_status(f"Reflection job queued for '{job_args['original_job_name']}'.", LYRN_SUCCESS)
    ...
    def open_job_watcher_popup(self):
        """Opens the Job Manager web interface."""
        webbrowser.open("http://localhost:8001/job_manager.html")
