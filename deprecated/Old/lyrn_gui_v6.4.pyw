"""
LYRN-AI Interface v6.4 - Advanced GUI with Full Feature Set
...
try:
    ...
    print("Warning: BlurWindow library not found. Glass theme will not work.")
    blur = None
...
class SystemResourceMonitor:
    ...
    def get_stats(self) -> Dict[str, any]:
        ...
        stats = {
            ...
            "cpu": 0, "cpu_temp": "N/A", "vram": 0, "hwinfo": None
        }
...
class TabbedSettingsDialog(ctk.CTkToplevel):
    ...
    def create_theme_builder_tab(self):
        ...
        self.glass_effect_var = ctk.BooleanVar()
        ctk.CTkCheckBox(left_frame, text="Enable Glass Effect", font=font, variable=self.glass_effect_var).pack(anchor="w", padx=10, pady=10)

        # --- Buttons ---
        button_frame = ctk.CTkFrame(left_frame)
        button_frame.pack(fill="x", padx=10, pady=10)
        preview_button = ctk.CTkButton(button_frame, text="Preview Changes", font=font, command=self.preview_theme)
        preview_button.pack(side="left", padx=10)
        Tooltip(preview_button, self.parent_app.tooltips.get("preview_theme_button", ""))
    ...
    def save_theme(self):
        ...
        if self.glass_effect_var.get():
            theme_data["special"] = {
                "blur": True,
                "transparent_color": self.color_entries["frame_bg"].get()
            }
        ...
        try:
            ...
            self.parent_app.theme_dropdown.configure(values=self.parent_app.theme_manager.get_theme_names())
...
class LyrnAIInterface(ctk.CTk):
    ...
    def setup_window(self):
        ...
        self.title("LYRN-AI Interface v6.4")
    ...
    def create_left_sidebar(self):
        ...
        try:
            if Image and ImageTk:
                logo_path = os.path.join(SCRIPT_DIR, "LYRN_LOGO.jpg")
                if os.path.exists(logo_path):
                    logo_image = Image.open(logo_path)
                    logo_photo = ImageTk.PhotoImage(logo_image.resize((32, 32)))
        ...
        self.settings_button = ctk.CTkButton(logo_frame, text="⚙", command=self.open_settings, width=30)
    ...
    def create_enhanced_status(self):
        ...
        self.status_frame = ctk.CTkFrame(self.left_sidebar, fg_color=self.theme_manager.get_color("primary", "#880ED4"))
...
"""
    ...
    def on_theme_selected(self, theme_name: str):
        ...
        # Handle special theme properties like glass
        self.apply_glass_effect()

    def apply_glass_effect(self):
        """Applies or removes the glass effect based on the current theme."""
        if not blur:
            return # Library not installed

        theme_data = self.theme_manager.themes.get(self.theme_manager.get_current_theme_name(), {})
        special_props = theme_data.get("special", {})

        if special_props.get("blur"):
            print("Applying glass effect...")
            transparent_color = special_props.get("transparent_color", "#1A202C")
            self.wm_attributes("-transparent", transparent_color)

            # Use a small delay to ensure the window is ready
            self.after(50, lambda: blur(self.winfo_id()))
        else:
            # Revert transparency if the new theme is not a blur theme
            self.wm_attributes("-transparent", "")
...
def main():
    """Main entry point for LYRN-AI v6.4"""
    log_queue = queue.Queue()
    redirector = ConsoleRedirector(log_queue)
    redirector.start()

    try:
        print("Starting LYRN-AI Interface v6.4...")
