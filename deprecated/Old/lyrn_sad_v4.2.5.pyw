...
class DraggableListbox(ctk.CTkScrollableFrame):
    ...
    def add_item(self, item_data: dict, **kwargs):
        ...
        delete_button = ctk.CTkButton(item_frame, text="🗑️", width=30, command=lambda: self.parent_popup.delete_component(item_data["path"]))
        delete_button.pack(side="right", padx=5)

        edit_button = ctk.CTkButton(item_frame, text="✏️", width=30, command=lambda: self.parent_popup.open_component_builder(item_data["path"]))
