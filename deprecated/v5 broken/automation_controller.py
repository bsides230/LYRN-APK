...
class AutomationController:
    ...
    """
    def __init__(self, job_definitions_path: str = "automation", queue_path: str = "automation/job_queue.csv"):
    ...
    def _load_job_definitions(self):
        """
        Loads job definitions from the jobs.csv file.
        """
        jobs_csv_path = self.job_definitions_path / "jobs.csv"
        self.job_definitions = {}
        if not jobs_csv_path.exists():
            print("No jobs.csv found. Creating default examples.")
            self._create_default_jobs()
            return

        try:
            with open(jobs_csv_path, 'r', encoding='utf-8', newline='') as f:
                reader = csv.DictReader(f)
                for row in reader:
                    name = row.get("Name", "").strip()
                    if name:
                        self.job_definitions[name] = {
            ...
            print(f"Loaded {len(self.job_definitions)} job definitions from jobs.csv: {list(self.job_definitions.keys())}")
        except IOError as e:
            print(f"Error loading job definitions from {jobs_csv_path}: {e}")
            self.job_definitions = {}

    def _create_default_jobs(self):
        """Creates a default jobs.csv file if none is found."""
        jobs_csv_path = self.job_definitions_path / "jobs.csv"
        try:
            with open(jobs_csv_path, 'w', encoding='utf-8', newline='') as f:
                writer = csv.writer(f)
                writer.writerow(["Name", "Trigger", "Sequence", "Description", "Meta", "Category"])
                writer.writerow(["summary_job", "", "", "Create a concise, factual summary of the provided text.", "", "default"])
                writer.writerow(["keyword_job", "", "", "Extract the main keywords.", "", "default"])
            print(f"Created default jobs file at {jobs_csv_path}")
            self._load_job_definitions()
        except IOError as e:
            print(f"Could not create default jobs file: {e}")

    def _read_queue_unsafe(self) -> List[Dict]:
        """Unsafely reads the job queue from the CSV file. Assumes lock is held."""
        queue = []
        try:
            if self.queue_path.exists() and self.queue_path.stat().st_size > 0:
                with open(self.queue_path, 'r', encoding='utf-8', newline='') as f:
                    reader = csv.DictReader(f)
                    for row in reader:
                        queue.append({
                            "id": row.get("ID", ""),
                            "name": row.get("Name", ""),
                            "priority": int(row.get("Priority", 100)) if row.get("Priority", "").isdigit() else 100,
                            "when": row.get("When", "now"),
                            "args": row.get("Args", "")
                        })
        except IOError as e:
    ...
    def _write_queue_unsafe(self, queue_data: List[Dict]):
        """Unsafely writes the job queue to the CSV file using an atomic operation. Assumes lock is held."""
        try:
            temp_path = self.queue_path.with_suffix(f"{self.queue_path.suffix}.tmp")
            with open(temp_path, 'w', encoding='utf-8', newline='') as f:
                writer = csv.DictWriter(f, fieldnames=["ID", "Name", "Priority", "When", "Args"])
                writer.writeheader()
    ...
    def save_job_definition(self, job_name: str, instructions: str, trigger: str = "", scripts: List[str] = None):
        """Legacy compatibility - no longer modifies jobs.csv to prevent data loss in Sequence column, but updates memory."""
        self.job_definitions[job_name] = {
            "trigger": trigger,
            "instructions": instructions,
            "sequence": ";".join([f"script:{s}" for s in scripts]) if scripts else ""
        }
        print(f"Warning: save_job_definition called for '{job_name}'. Modified in memory only, CSV untouched.")

    def delete_job_definition(self, job_name: str):
        """Legacy compatibility."""
        if job_name in self.job_definitions:
            del self.job_definitions[job_name]
        print(f"Warning: delete_job_definition called for '{job_name}'. Modified in memory only, CSV untouched.")
    ...
    def add_job(self, name: str, priority: int = 100, when: str = "now", args: Optional[Dict[str, Any]] = None, job_id: Optional[str] = None):
        ...
        # Allow running .py scripts directly or defined jobs
        if name not in self.job_definitions and not name.endswith(".py") and not name.startswith("affordance:"):
            print(f"Warning: Job '{name}' not defined in jobs.csv and is not a direct script/affordance. Cannot add to queue.")
            return
        ...
        try:
            with SimpleFileLock(self.queue_lock_path):
                ...
                if due_idx != -1:
                    ...
                    job_name = next_job_dict["name"]

                    if job_name.startswith("affordance:"):
                        # Execute an affordance (like FINAL_OUTPUT) by writing the trigger file
                        affordance_name = job_name.split(":", 1)[1]
                        import sys
                        # Fallback for sequence_executor affordance step
                        if affordance_name == "FINAL_OUTPUT":
                            print(f"[Scheduler] Affordance '{affordance_name}' handled by chat_watcher_bg.py, ignoring in queue.")
                        else:
                            # Or handle via prompt / scripts as needed
                            print(f"[Scheduler] Processing affordance: {affordance_name}")
                    ...
                    elif job_name.endswith(".py") and job_name not in self.job_definitions:
                    ...
                    # Normal Job Definition processing
                    try:
                        args_dict = json.loads(next_job_dict.get("args", "")) if next_job_dict.get("args") else {}
                    except json.JSONDecodeError:
                        args_dict = {}

                    instruction_prompt = self.get_job_instructions_prompt(job_name, args_dict)
                    if instruction_prompt is None:
                        instruction_prompt = ""

                    # Fetch script list from definition sequence if there are any old styles
                    job_def = self.job_definitions.get(job_name, {})

                    scripts = []
                    # For compatibility, if they set sequence to script list
                    seq = job_def.get("sequence", "")
                    if seq:
                        for part in seq.split(";"):
                            if part.startswith("script:"):
                                scripts.append(part.split(":")[1])

                    if not scripts and "scripts" in job_def: # fallback to memory
